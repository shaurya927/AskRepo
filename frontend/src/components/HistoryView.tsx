    ...
    <div className="space-y-4">
            ...
            <div className="bg-white dark:bg-[#161b22] border border-gray-200 dark:border-[#30363d] rounded-lg p-4">
              ...
              <div className="flex items-end gap-[2px] h-16">
                {timeline.map((t, i) => (
                  <div
                    key={i}
                    className="flex-1 bg-green-500 dark:bg-[#238636] rounded-t-sm hover:bg-green-600 dark:hover:bg-[#2ea043] transition-colors cursor-default"
              ...
              <div className="flex justify-between mt-1">
                <span className="text-[10px] text-gray-400 dark:text-[#6e7681]">{timeline[0]?.week}</span>
                <span className="text-[10px] text-gray-400 dark:text-[#6e7681]">{timeline[timeline.length - 1]?.week}</span>
              </div>
            </div>
          )}
                  ...
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex items-start gap-2 min-w-0">
                        {expandedSha === c.sha ? <ChevronDown size={14} className="text-gray-500 dark:text-[#8b949e] mt-0.5 flex-shrink-0" /> : <ChevronRight size={14} className="text-gray-500 dark:text-[#8b949e] mt-0.5 flex-shrink-0" />}
                        <div className="min-w-0">
                          <p className="text-sm text-gray-900 dark:text-[#e6edf3] truncate">{c.message.split('\n')[0]}</p>
                          <div className="flex items-center gap-2 mt-1">
                            <span className="text-xs font-mono text-blue-600 dark:text-[#58a6ff]">{c.sha.substring(0, 7)}</span>
                            <span className="text-xs text-gray-500 dark:text-[#8b949e]">{c.author_name}</span>
                            <span className="text-xs text-gray-400 dark:text-[#6e7681]">{new Date(c.authored_date).toLocaleDateString()}</span>
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center gap-2 flex-shrink-0 text-xs">
                        <span className="text-green-600 dark:text-[#3fb950]"><Plus size={10} className="inline" />{c.insertions}</span>
                        <span className="text-red-500 dark:text-[#f85149]"><Minus size={10} className="inline" />{c.deletions}</span>
                        <span className="text-gray-500 dark:text-[#8b949e]">{c.files_changed} files</span>
                      </div>
                    </div>
                  </button>
                      ...
                      {loadingDetail ? (
                        ...
                        <div className="space-y-2">
                          {commitDetail.message.includes('\n') && (
                            <p className="text-xs text-gray-500 dark:text-[#8b949e] whitespace-pre-wrap mb-3">
                            ...
                            <div key={i} className="flex items-center justify-between text-xs py-1">
                              <div className="flex items-center gap-2 min-w-0">
                                <span className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${
                                  fc.change_type === 'added' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' :
                                  fc.change_type === 'deleted' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' :
                                  fc.change_type === 'renamed' ? 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400' :
                                  'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400'
                                }`}>
                                  {fc.change_type.charAt(0).toUpperCase()}
                                </span>
                                <span className="font-mono text-gray-900 dark:text-[#e6edf3] truncate">{fc.file_path}</span>
                              </div>
                              <div className="flex gap-2 flex-shrink-0">
                                <span className="text-green-600 dark:text-[#3fb950]">+{fc.insertions}</span>
                                <span className="text-red-500 dark:text-[#f85149]">-{fc.deletions}</span>
                              </div>
                            </div>
                          ))}
                        </div>
                      ) : (
                        <p className="text-xs text-gray-400 dark:text-[#6e7681]">Failed to load details</p>
                      )}
                    </div>
                  )}
                </div>
                ...
                return (
                  <div key={i} className="px-4 py-2.5 flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-3 flex-shrink-0 text-xs">
                      <span className="text-gray-900 dark:text-[#e6edf3] font-medium">{h.change_count}×</span>
                      <span className="text-green-600 dark:text-[#3fb950]">+{h.total_insertions}</span>
                      <span className="text-red-500 dark:text-[#f85149]">-{h.total_deletions}</span>
                    </div>
                  </div>
                )
              })}
      ...
      {subTab === 'co-changes' && (
        <div className="bg-white dark:bg-[#161b22] border border-gray-200 dark:border-[#30363d] rounded-lg overflow-hidden">
          ...
          {coChanges.length === 0 ? (
            <div className="p-8 text-center text-gray-500 dark:text-[#8b949e] text-sm">No co-change data available.</div>
          ) : (
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-[#21262d]">
                {coChanges.slice(0, 30).map((cc, i) => (
                  <tr key={i} className="hover:bg-gray-50 dark:hover:bg-[#1c2128]">
                    ...
                    <td className="px-4 py-2 text-xs font-mono text-gray-900 dark:text-[#e6edf3] truncate max-w-[200px]">{cc.file_b}</td>
                    <td className="px-4 py-2 text-xs text-blue-600 dark:text-[#58a6ff] text-right font-medium">{cc.co_change_count}</td>
                  </tr>
                ))}
              </tbody>
            </table>
